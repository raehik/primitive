...
class Prim a where
  ...
  -- | Read a value from the mutable array. The offset is in bytes.
  readWord8ByteArrayAs#
      :: MutableByteArray# s -> Int# -> State# s -> (# State# s, a #)

  -- | Write a value to the mutable array. The offset is in bytes.
  ...
  {-# MINIMAL (sizeOfType# | sizeOf#), (alignmentOfType# | alignment#), indexByteArray#, readByteArray#, writeByteArray#,
    indexWord8ByteArrayAs#, readWord8ByteArrayAs#, writeWord8ByteArrayAs#,
...
instance Prim a => Prim (Complex a) where
  ...
  indexWord8ByteArrayAs# arr# i# =
    let x = indexWord8ByteArrayAs# arr# (2# *# i#)
        y = indexWord8ByteArrayAs# arr# (2# *# i# +# sizeOf# (undefined :: a))
    in x :+ y
  readWord8ByteArrayAs# arr# i# =
    \s0 -> case readByteArray# arr# (2# *# i#) s0 of
       (# s1#, x #) -> case readByteArray# arr# (2# *# i# +# sizeOf# (undefined :: a)) s1# of
          (# s2#, y #) -> (# s2#, x :+ y #)
  writeWord8ByteArrayAs# arr# i# (a :+ b) =
    \s0 -> case writeWord8ByteArrayAs# arr# (2# *# i#) a s0 of
       s1 -> case writeByteArray# arr# (2# *# i# +# sizeOf# (undefined :: a)) b s1 of
         s2 -> s2
  indexOffAddr# addr# i# =
...
#define derivePrim(ty, ctr, sz, align, idx_arr, rd_arr, wr_arr, set_arr, idx_arr_w8, rd_arr_w8, wr_arr_w8, idx_addr, rd_addr, wr_addr, set_addr) \
...
; setByteArray# arr# i# n# (ctr x#) s#                            \
    = let { i = fromIntegral (I# i#)                              \
      ...
      case unsafeCoerce# (internal (set_arr arr# i n x#)) s# of   \
        { (# s1#, _ #) -> s1# }                                   \
; indexWord8ByteArrayAs# arr# i# = ctr (idx_arr_w8 arr# i#)       \
; readWord8ByteArrayAs#  arr# i# s# = case rd_arr_w8 arr# i# s# of \
                        { (# s1#, x# #) -> (# s1#, ctr x# #) }    \
; writeWord8ByteArrayAs# arr# i# (ctr x#) s# = wr_arr_w8 arr# i# x# s# \
                                                                  \
...
derivePrim(Float, F#, sIZEOF_FLOAT, aLIGNMENT_FLOAT,
           ...
           indexWord8ArrayAsFloat#, readWord8ArrayAsFloat#, writeWord8ArrayAsFloat#,
...
derivePrim(StablePtr a, StablePtr, sIZEOF_PTR, aLIGNMENT_PTR,
           ...
           indexWord8ArrayAsStablePtr#, readWord8ArrayAsStablePtr#, writeWord8ArrayAsStablePtr#,
...
instance Prim WordPtr where
  ...
  readWord8ByteArrayAs# a i s0 = case readWord8ByteArrayAs# a i s0 of
    (# s1, p #) -> (# s1, ptrToWordPtr p #)
  writeWord8ByteArrayAs# a i wp = writeWord8ByteArrayAs# a i (wordPtrToPtr wp)
...
instance Prim IntPtr where
  ...
  readWord8ByteArrayAs# a i s0 = case readWord8ByteArrayAs# a i s0 of
    (# s1, p #) -> (# s1, ptrToIntPtr p #)
  writeWord8ByteArrayAs# a i wp = writeWord8ByteArrayAs# a i (intPtrToPtr wp)
